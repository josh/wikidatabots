...
def lookup_itunes_id(expr: pl.Expr, batch_size: int = _LOOKUP_BATCH_SIZE) -> pl.Expr:
    return expr.map(
        partial(_lookup_itunes_id, batch_size=batch_size),
...
def _lookup_itunes_id(s: pl.Series, batch_size: int) -> pl.Series:
    return (
        s.to_frame("id")
        .lazy()
        .select(
