...
def test_formatted_url():
    ...
    assert imdb_id.formatted_url("ch0000985") == None
    assert imdb_id.formatted_url("co0071326") == None
    assert imdb_id.formatted_url("tt11696836/characters/nm11012957") == None
    assert imdb_id.formatted_url("0000399") == None


def externalid_url(id):
    url = "https://wikidata-externalid-url.toolforge.org/?p=345&url_prefix=https://www.imdb.com/&id={}".format(
        id
    )
    r = requests.head(url)
...
def test_extract_id():
    ...
    assert (
        imdb_id.extract_id("https://www.imdb.com/search/title/?companies=co0071326")
        == None
    )
